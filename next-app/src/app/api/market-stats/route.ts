...
const EXCLUDE_KEYWORDS = [
  ...
  '곱도리', '누룽지', // 곱도리탕·백숙/누룽지탕/누룽지 제조판매(2026-09-01 실측 42상호, 카페 오탐 0 — 커피숍 인허가 4건도 전부 누룽지 전문점)
];
...
const EXCLUDE_ADDR_KEYWORDS = [
  ...
  '축제', // 주소가 '○○축제장·축제행사장'인 노점(992행 실측) — 상호엔 축제가 없어 이름 키워드를 통과하던 것
];
...
function isTarget(item: any): boolean {
  ...
  if (isNonTargetName(bizName, areaM2)) return false; // 치킨 브랜드·한식(100평 미만)·중식·일식·아시아 — 상호 규칙(2026-09-04 4,120행 실측)
  ...
  if (cat === '한식' && areaM2 < 330) return false;
  return true;
}
