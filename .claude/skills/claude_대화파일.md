...
| v109 | PC 프로필 드롭다운 + 설정 모달 + 색각 보정 모드 (적녹색맹 초록→청록, 빨강→마젠타) |
| v110 | 사이드 필터 기본 접힘 + 섹션명 변경 + 색각 보정 필터 색상 연동 + 전체탭 주요거래처현황 버그 수정 |
| v111 | 방문일지 업로드 409 폴백 처리: 배치 INSERT 충돌 시 행별 개별 INSERT로 재시도, 충돌 행만 건너뜀 (showToast 경고) |
...
| ~~proposal.html 제품DB Supabase 전환~~ | ✅ 완료 — Cloudflare R2로 전환 결정 및 적용 |
...
## 2026-03-17 — 설정 모달 + 색각 보정 모드 (v109)

### 사업부장(수도권FS지역사업부장) 뷰 설계 논의 → 보류

- 핵심 판단: 담당자별 활동량(방문 건수 등) 데이터를 사업부장에게 노출하는 것은 **감시 도구**가 될 수 있어 팀 문화상 바람직하지 않음
- 사업부장에게 필요한 것은 **인허가/거래처 현황 집계 (지점별)** — 사람이 아닌 사업 상태
- 거창한 전용 페이지보다 기존 페이지의 데이터 범위를 3개 지점으로 확장하되 개인 식별 정보 미노출 방향
- **보류**: 당장 우선순위 아님, 실사용 요구 생기면 재논의

### 설정 모달 + 색각 보정 모드 (SW v109)

**흐름:**
- PC: nav 이름 클릭 → 프로필 드롭다운 (📅 내 일정 / ⚙️ 설정 / 로그아웃)
- 모바일: 하단 프로필 팝업에 ⚙️ 설정 버튼 추가
- 설정 모달 내 "색각 보정 모드" 토글 → `localStorage('fs_colorblind_mode')` 저장 → 리로드 시 `body.colorblind` 클래스 적용

**색상 변경 (보정 모드 ON):**
| 상태 | 기본 | 보정 모드 |
|------|------|---------|
| 인허가 마커 | 초록 `#34C759` | 청록 `#0099CC` |
| 미거래 마커 | 빨강 `#FF3B30` | 마젠타 `#CC0099` |
| status-card 보더 | 동일 | 동일하게 적용 |
| 토스트 success/error | 동일 | 색상 조정 |

**수정 파일:** `js/nav-component.js`, `common.css`, `index.html`
...
- **필터 기본 접힘**: 3개 드롭다운(`ddStatusCard`, `ddMilkCard`, `ddAccountCard`) 초기 `open` 클래스 제거 → 화살표 클릭 시만 펼침
- **섹션명 변경**: 거래 현황→인허가 현황 / 사용우유→사용우유(인허가) / 거래/미거래 현황→주요거래처 현황
- **색각 보정 필터 연동**: `_statusItems`와 `_milkColors`에 `_isColorblind` 분기 추가
  - 인허가(초록→청록), 미거래(빨강→마젠타), 서울우유(빨강→마젠타), 연세우유(초록→청록)
- **모바일 레이어 이모지 교체**: 색각 보정 시 🟢→🔵(인허가), 🔴→🟣(미거래)
- **버그 수정**: 전체 탭 초기 로드 시 주요거래처현황 필터 미표시
  - 원인: `sidebarContentAccount` 초기 `active` 클래스 누락 (JS는 클릭 시만 적용)
  - 수정: HTML 초기값에 `active` 클래스 추가
- **커밋**: `80868c7` (v110), `a38017b` (버그 수정)
