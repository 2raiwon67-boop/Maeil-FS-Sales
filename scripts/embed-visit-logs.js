...
async function main() {
    ...
    const res = await fetch(
        `${SUPABASE_URL}/rest/v1/visit_logs?select=id,business_name,visit_date,manager,content,trade_status&embedding=is.null&or=(content.ilike.*개척완료*,content.ilike.*개척 완료*,content.ilike.*연결완료*)&order=visit_date.desc&limit=${MAX_PER_RUN}`,
        { headers: sbHeaders }
    );
